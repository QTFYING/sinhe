...
本清单只负责回答“当前实现与目标模型差多少，以及应该如何处置”，不负责重新定义 API 语义。
...
为避免编码期再次回到“看到旧代码就顺手补丁”的路径，本清单进一步给出目录级处置建议：

1. `apps/api/src/auth`
   保留并校准；允许继续作为当前认证底座。
2. `apps/api/src/order`
   现状仅作旧实现参考，不继续承载新订单主链路。
3. `apps/api/src/import`
   现状仅作历史导入样例，不继续沿用旧字段和旧唯一键。
4. `apps/api/src/payment`
   现状只可参考基础接入方式，不继续沿用旧 `payStatus` 主流程。
5. `apps/api/src/print`
   现状不再作为主线；后续仅围绕打印配置和打印回执重建。
6. `apps/api/src/notification`、`apps/api/src/report`
   在主链路稳定前冻结；必要时优先隔离编译影响。

## 八、执行判定规则

后续进入真实编码时，遇到实现选择分歧，按以下顺序判断：

1. 若模块机制可复用但业务语义已漂移，按“复用骨架 + 重写业务”处理。
2. 若数据模型无法支撑当前文档，优先重建 schema，不在 service 层硬补。
3. 若旧实现只是字段名不同而语义已变，不视为可兼容演进对象。
4. 若外围域依赖主链路事实表，主链路未定稿前一律暂缓。
