...
6. 当前 `pnpm -F api build` 已通过，说明核心实现已具备继续联调和扩展的基础。
7. 当前剩余问题已不再是“主链路是否存在”，而是：
   - Swagger / OpenAPI 尚未系统化同步
   - Admin 与外围域仍未按新契约完成
   - 个别链路仍有阶段性技术债待收口
...
| `M7` | 平台侧与外围域收口 | 进行中 | `notification / report` 已最小收口，Admin 与外围域仍待补齐 |
| `M8` | 联调产物与质量收口 | 待开始 | Swagger、系统化验证、技术债回补仍待推进 |
...
1. 平台侧与租户侧边界明确
2. 主链路不会再被外围域带回旧字段或旧状态机
3. 需要跨租户读取的数据只出现在 Admin 侧
...
1. Auth、Settings、Import Template、Import、Orders、Payment、Print Records 的 Swagger 同步
2. 主链路最小接口自测与错误场景复核
3. 技术债优先项，例如：
   - `TD-005` 催款提醒持久化
   - 导入可靠性改造
...
1. Swagger 路径、请求体、响应体与 `docs/api`、contracts 一致
2. 已完成域具备基本的联调与回归依据
3. 技术债被显式纳入后续排期，而不是继续隐含存在
...
1. 已完成主链路若未及时同步 Swagger，后续联调仍会出现口径漂移。
2. Admin 与外围域若直接复制租户侧实现，容易造成租户边界污染。
3. `reminder`、导入队列等技术债若长期不收口，会在进入测试环境后集中暴露。
4. 若继续保留“文档已更新但 design 未同步”的状态，后续执行优先级会再次混乱。
...
2. `TD-005` 催款提醒持久化建模与接口收口
3. Admin 订单 / 支付 / 对账只读能力
4. 资质审核、租户审核等平台审核链路
5. 导入可靠性技术债，例如任务队列化与预检快照策略
6. 关键链路的最小自测、回归与联调记录
