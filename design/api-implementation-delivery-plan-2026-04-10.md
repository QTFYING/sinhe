...
本手册用于定义事实源顺序、实施边界、阶段目标、统一验收标准与存档规则。
凡与后续实现顺序、模块优先级、旧链路清退范围有关的判断，以本手册为准。
...
1. `design` 目录下 3 份手册均视为当前阶段生效文档，不再作为纯讨论稿使用。
2. 若后续需要修订，必须直接更新原文件，不再平行新增“临时版”“讨论版”“最终版”。
3. 若新增第二批、第三批执行清单，应继续存放于 `design` 目录，并明确对应里程碑与生效日期。
...
1. 受影响事实源的校对结果
   即 `docs/api`、`enums`、`contracts`、`data-model-reference` 中与本阶段相关的部分已确认一致。
2. 代码实现
   至少包含当前阶段对应的 controller、service、dto、schema 或模块装配。
3. 最小验证记录
   至少说明已完成的类型检查、构建验证、接口自测或编译范围。
4. 遗留项记录
   若有明确暂缓项、受旧模块影响的残留项，必须在阶段收口时写明，而不是隐式留待后续发现。

## 七、阶段交付件约束

为避免“只改代码不改契约”或“只改类型不改实现”，每个阶段的交付件最低要求如下：

1. 总纲层
   若阶段目标、边界、优先级发生变化，先修订本手册。
2. 差异层
   若发现新的复用/改造/重写判断，需要同步修订差异清单。
3. 执行层
   当前批次若已进入实施，必须同步维护对应执行清单。
4. 实现层
   只允许按事实源落代码，不允许实现先行倒逼文档改义。
5. 产物层
   Swagger / OpenAPI、测试结果、构建结果均属于产物，不得反向成为事实源。
...
旧 MVP 模块在实施期间必须采用“冻结 / 局部复用 / 重写替换”的明确策略，避免新旧链路混写。

处置原则如下：

1. `auth`
   允许复用基础机制，但只做校准，不扩散新的历史兼容分支。
2. `order / import / payment / print`
   视为待重写主域，不继续在旧 service 上叠加新业务语义。
3. `notification / report`
   视为暂缓域；若影响当前阶段编译或装配，应优先隔离，而不是被动修补。
4. 旧字段、旧接口、旧状态机
   只允许作为迁移识别对象存在，不允许重新进入新 contracts、新 controller、新 schema。
5. 新阶段启动前
   先确认本阶段允许触达的目录范围，避免订单、支付、打印三域同时无边界扩散。
