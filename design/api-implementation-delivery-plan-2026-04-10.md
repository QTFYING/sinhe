...
| `M8` | 联调产物与质量收口 | 已完成 | Swagger 第一轮、主链路补强与最小验证已完成 |
...
1. Swagger 已完成第一轮按模块分组和接口摘要同步
2. 主链路已具备 build 级验证依据
...
1. 当前 Swagger 仍是第一轮同步，后续若响应 schema 继续细化不及时，仍会产生漂移。
2. 导入队列化、预检快照等可靠性问题仍会在测试环境集中暴露。
