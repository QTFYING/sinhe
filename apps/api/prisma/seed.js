...
async function main() {
  ...
  if (!osAdmin) {
    osAdmin = await prisma.user.create({
      data: { username: 'admin', passwordHash: defaultHash, realName: 'OS超级运营管理员', role: 'OS_SUPER_ADMIN', status: 1 }
    });
  } else {
  ...
  if (!tenant) {
    tenant = await prisma.tenant.create({
      data: { name: '华东区饮料总代(测试)', contactPhone: '13888888888', maxCreditDays: 45, creditReminderDays: 7, status: 1 }
    });
  }
  ...
  if (!boss) {
    boss = await prisma.user.create({
      data: { tenantId: tenant.id, username: 'boss', passwordHash: defaultHash, realName: '经销商李老板', role: 'TENANT_OWNER', status: 1 }
    });
  } else {
