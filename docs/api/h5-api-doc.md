...
> 枚举命名与取值统一参见 **[../enums/enum-manual.md](../enums/enum-manual.md)**，本档仅保留 H5 支付相关的最小定义。
...
- **GET** `/pay/:token`
- **描述**：根据订单公开路由标识获取支付页面所需的完整订单信息，包括商户、客户、商品明细、支付状态等
- **是否鉴权**：否（公开页面路由）
...
```typescript
{
  ...
  selectedPaymentMethod: PaymentMethod | null // 当前已选择的支付方式
...
- **描述**：请求后端发起在线支付。后端将向拉卡拉统一下单，并返回原生收银台转跳链接。
- **是否鉴权**：否
...
```typescript
{
  ...
  selectedPaymentMethod?: PaymentMethod // 当前已选择的支付方式
}
```
...
```typescript
type PaymentMethod = 'online' | 'cash' | 'other_paid'
```
...
type CashVerifyStatus = 'pending' | 'verified'
```
...
interface OfflinePaymentInfo {
  ...
  remark: string                   // 备注
  cashVerifyStatus: CashVerifyStatus | null  // 核销状态（仅现金支付）
