...
7. [订单管理 Orders](#七订单管理-orders10-个端点)
...
```typescript
type OrderStatus = 'paid' | 'partial' | 'pending' | 'expired' | 'credit'
...
| 租户中心 Tenant Center | 10 | GET ×3, POST ×7 | tenants, tenant_certifications |
| 用户管理 Users | 6 | GET ×1, POST ×3, PUT ×1, DELETE ×1 | users |
| 订单管理 Orders | 10 | GET ×4, POST ×5, PUT ×1 | orders, import_templates, import_jobs |
...
| **合计** | **89** | GET ×36, POST ×33, PUT ×12, DELETE ×8 | — |
...
| 移除 `/orders/import/confirm`（-1） | 订单导入统一一步完成，字段映射前端处理 |
